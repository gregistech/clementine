...
class config_handler:
    def get_default_config(key):
        return db_handler.fetch_query(Query("SELECT {field} FROM configs WHERE id = ?".format(field=key), [0]))[0]

    def get_config(guild_id, key):
        try:
            return db_handler.fetch_query(Query("SELECT {field} FROM configs WHERE id = ?".format(field=key), [guild_id]))[0]
    ...
    def set_config(guild_id, key, value):
        ...
        db_handler.execute_query(Query("UPDATE configs SET {field} = ? WHERE id = ?".format(field=key), (value, guild_id)))
